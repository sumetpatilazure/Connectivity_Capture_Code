            ...
            if ($Resource.ResourceType -eq "Microsoft.Network/virtualNetworks" ) {
                ...
                foreach ($VirtualNetworkPeering in $VirtualNetworkPeerings) {
                    ...
                    elseif ($VirtualNetworkPeering.peeringSyncLevel -contains "RemoteNotInSync") {
                        Write-Output ("Please check the remote peering as this Tenant does not have any subscription with the LocalNotInSync status " )
                    }
                    else {
                        Write-Output ("No resources were found with the LocalNotInSync status everything is updated" )
                    }
                }
            }
        }
